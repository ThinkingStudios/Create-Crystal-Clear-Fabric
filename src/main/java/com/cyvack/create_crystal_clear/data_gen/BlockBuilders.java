...
public class BlockBuilders {
	...
	// Entry
	public static BlockEntry<TintedGlassCasing> tintedglasscasing(String name,
	...
	// Entry
	public static BlockEntry<GlassEncasedShaftBlock> glassEncasedShaft(String casingType, Boolean clear,
			NonNullFunction<Properties, GlassEncasedShaftBlock> factory) {
		String name = clear ? casingType + "_clear" : casingType;
		return REGISTRATE
				.block(name + "_glass_encased_shaft", factory)
				.transform(
						glassEncasedShaftBuilder(name + "_glass", () -> ModSpriteShifts.omni(name + "_glass_casing")))
				.register();
	}

	// Builder
	public static <B extends GlassEncasedShaftBlock, P> NonNullUnaryOperator<BlockBuilder<B, P>> glassEncasedShaftBuilder(
	...
	// Entry
	public static BlockEntry<GlassEncasedCogwheel> glassEncasedCogwheel(String casingType, Boolean large, Boolean clear,
			NonNullFunction<Properties, GlassEncasedCogwheel> factory) {
		String name = clear ? casingType + "_clear" : casingType;
		return !large ?
		// small cog
				REGISTRATE
						...
						.onRegister(CreateRegistrate.connectedTextures(() -> new EncasedCogCTBehaviour(
								...
								Couple.create(
										ModSpriteShifts
												.vertical("encased_cogwheels/" + casingType + "_encased_cogwheel_side"),
										ModSpriteShifts.horizontal(
												"encased_cogwheels/" + casingType + "_encased_cogwheel_side")))))
						.register()
				:
				// Large Cog
				REGISTRATE
	...
	private static <B extends GlassEncasedCogwheel, P> BlockBuilder<B, P> glassEncasedCogwheelBase(BlockBuilder<B, P> b,
			String casingType, Boolean clear, Supplier<CTSpriteShiftEntry> casingShift, Supplier<ItemLike> drop,
			boolean large) {
		...
		return glassencasedBase(b, drop)
				...
				.onRegister(CreateRegistrate.casingConnectivity((block, cc) -> cc.make(block, casingShift.get(),
						(s, f) -> f.getAxis() == s.getValue(GlassEncasedCogwheel.AXIS)
								&& !s.getValue(f.getAxisDirection() == Direction.AxisDirection.POSITIVE
										? GlassEncasedCogwheel.TOP_SHAFT
				...
				.blockstate((c, p) -> axisBlock(c, p, blockState -> {
					...
					return p.models()
							...
							// Gearbox
							.texture(large ? "3" : "4",
									casingType.equals("andesite") ? Create.asResource("block/gearbox")
											: getGearbox(casingType + "_"))
							// Side Casing
	...
	private static ResourceLocation getGearbox(String opening) {
		return opening.equals("brass_") ? Create.asResource("block/" + opening + "gearbox")
